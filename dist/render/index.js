...
export function resetWidthCache() {
    _cachedWidth = null;
    _cachedWidthAt = 0;
}
...
const HOLD_MS = 3000;
function getHighWaterPath() {
    return path.join(os.homedir(), '.claude', 'plugins', 'claude-hud', '.line-highwater');
}
function readHighWater() {
    try {
        const data = JSON.parse(fs.readFileSync(getHighWaterPath(), 'utf8'));
        if (typeof data.highWater !== 'number' || typeof data.lastHighAt !== 'number')
            return null;
        return data;
    }
    catch {
...
function getPaddedLineCount(actual) {
    const now = Date.now();
...
export function clearHighWater() {
    try {
...
function renderCompact(ctx, maxWidth) {
    const lines = [];
