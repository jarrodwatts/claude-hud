...
test('getModelName trims id fallback and maps blank id to Unknown', () => {
  assert.equal(
    getModelName({ model: { display_name: '  ', id: '  sonnet-456  ' } }),
    'sonnet-456'
  );
  assert.equal(
    getModelName({ model: { display_name: '  ', id: '   ' } }),
    'Unknown'
  );
});
...
test('getModelName normalizes exact canonical opus 4.6 id', () => {
  assert.equal(
    getModelName({ model: { id: 'anthropic.claude-opus-4-6-v1' } }),
    'Opus 4.6'
  );
});

test('getModelName normalizes global-prefixed bedrock id', () => {
  assert.equal(
...
test('getModelName normalizes exact documented sonnet 4.6 id', () => {
  assert.equal(
    getModelName({ model: { id: 'anthropic.claude-sonnet-4-6' } }),
    'Sonnet 4.6'
  );
});

test('getModelName normalizes documented sonnet 4.5 id', () => {
  assert.equal(
    getModelName({ model: { id: 'anthropic.claude-sonnet-4-5-20250929-v1:0' } }),
    'Sonnet 4.5'
  );
});

test('getModelName normalizes documented sonnet 4 id', () => {
  assert.equal(
    getModelName({ model: { id: 'anthropic.claude-sonnet-4-20250514-v1:0' } }),
    'Sonnet 4'
  );
});

test('getModelName normalizes documented sonnet 3.7 id', () => {
  assert.equal(
    getModelName({ model: { id: 'anthropic.claude-3-7-sonnet-20250219-v1:0' } }),
    'Sonnet 3.7'
  );
});

test('getModelName normalizes documented opus 4.5 id', () => {
  assert.equal(
    getModelName({ model: { id: 'anthropic.claude-opus-4-5-20251101-v1:0' } }),
    'Opus 4.5'
  );
});
...
test('getModelName normalizes documented opus 4 id', () => {
  assert.equal(
    getModelName({ model: { id: 'anthropic.claude-opus-4-20250514-v1:0' } }),
    'Opus 4'
  );
});

test('getModelName normalizes documented haiku 4.5 id', () => {
  assert.equal(
    getModelName({ model: { id: 'anthropic.claude-haiku-4-5-20251001-v1:0' } }),
    'Haiku 4.5'
  );
});
...
test('getModelName normalizes documented haiku 3 id', () => {
  assert.equal(
    getModelName({ model: { id: 'anthropic.claude-3-haiku-20240307-v1:0' } }),
    'Haiku 3'
  );
});
...
test('getModelName does not normalize wrong-version tuples', () => {
  const raw = 'anthropic.claude-opus-4-9-20251101-v1:0';
  assert.equal(getModelName({ model: { id: raw } }), raw);
});
...
test('getModelName keeps non-bedrock ids unchanged', () => {
  const raw = 'claude-3-5-sonnet-20241022';
  assert.equal(getModelName({ model: { id: raw } }), raw);
});
