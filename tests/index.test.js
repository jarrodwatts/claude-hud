...
test('main includes git status in render context', async () => {
  ...
  await main({
    ...
    loadConfig: async () => ({
      ...
      display: { showModel: true, showContextBar: true, contextValue: 'percent', showConfigCounts: true, showDuration: true, showTokenBreakdown: true, showUsage: true, showTools: true, showAgents: true, showTodos: true, autocompactBuffer: 'enabled', usageThreshold: 0, sevenDayThreshold: 80, environmentThreshold: 0 },
    }),
    render: (ctx) => {
      renderedContext = ctx;
    },
