...
| `display.showConfigCounts` | boolean | false | Show CLAUDE.md, rules, MCPs, hooks counts. CLAUDE.md detection mirrors Claude Code's memory discovery: managed-policy + `~/.claude/CLAUDE.md` + every `CLAUDE.md`/`CLAUDE.local.md` walking from the working directory up to the filesystem root (so a monorepo-root CLAUDE.md above your cwd is included), honoring `claudeMdExcludes` glob patterns from your settings (managed-policy excepted). |
| `display.showConfigPaths` | boolean | false | Append the abbreviated path of each detected CLAUDE.md after the count, e.g. `3 CLAUDE.md (~/.claude/CLAUDE.md, ~/repo/CLAUDE.md, ./CLAUDE.md)`. Requires `showConfigCounts`. |
