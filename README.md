...
| `projectLineOrder` | string[] | `["model","project","advisor","sessionName","version","extra","duration","cost","speed","auth"]` | Order of segments *within* the first (identity/project) line, in both layouts. Reordering only — visibility stays with the `display.show*` flags, and omitted segments are appended in default order. `model` covers provider + model + effort (plus the context bar in compact mode); `project` covers path + added dirs + git as one segment. Example: `["project","model"]` puts the project/git block before the model badge. |
