...
| `display.pricingOverrides` | array | `[]` | User pricing rules matched before the bundled table. Each entry is `{ "pattern": "<regex>", "inputUsdPerMillion": <n>, "outputUsdPerMillion": <n> }`; `pattern` is a case-insensitive regex tested against the normalized model name (lowercased, separators collapsed to spaces, e.g. `opus 4 6`). Invalid entries are dropped |
...
Pricing for the estimate comes from a bundled `pricing.json` (per-token rates matching published Anthropic/Bedrock pricing, including the reduced Opus 4.5+ and Haiku 4.5 rates). You can extend or correct it without rebuilding via `display.pricingOverrides`, which are matched before the bundled rules.
