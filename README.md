...
| `display.modelSource` | `auto` \| `stdin` \| `transcript` | `auto` | Controls which source the model name comes from. `auto` (default) uses the stdin model for Claude models and the transcript model for non-Claude (proxy redirect detection). `stdin` always uses what Claude Code reports. `transcript` always uses the model from the API response — best for proxy users (cc-switch, LiteLLM, etc.) who want the actual served model |
