...
| `editorUriScheme` | string | `vscode` | URI scheme used by the `display.showOpenInEditor` link, e.g. `vscode`, `cursor`, `windsurf`. Must match `^[a-z][a-z0-9+.-]*$`; invalid values fall back to the default |
...
| `display.showOpenInEditor` | boolean | false | Add a clickable `[Code]` link that opens the agent's *current* working directory in a new editor window via `editorUriScheme`. Prefers `workspace.current_dir`/`workspace.git_worktree` over `cwd`, so it follows the agent into a git worktree or after a `cd` instead of staying pinned to the directory Claude Code was launched from |
