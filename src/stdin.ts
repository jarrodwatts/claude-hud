...
export function getSessionCost(stdin: StdinData): number {
 ...
 */
export function getCacheRatio(stdin: StdinData): number | null {
 ...
 */
export function getEfficiencyZone(contextPercent: number, cacheRatio: number | null): 'green' | 'yellow' | 'red' {
  const cache = cacheRatio ?? 100; // assume good if unknown

  if (contextPercent > 80) return 'red';
  if (contextPercent > 60 && cache < 70) return 'red';
  if (contextPercent > 60) return 'yellow';
  if (contextPercent > 40 && cache < 80) return 'yellow';
  return 'green';
}
